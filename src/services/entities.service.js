...
const users = {
  ...
  create: async (clinicId, { name, email, role_id }) => {
    const existing = await pool.query('SELECT id FROM users WHERE email = $1 AND clinic_id = $2', [
      email,
      clinicId,
    ]);
    if (existing.rows[0])
      throw new ConflictError('A user with this email already exists in this clinic');
...
const staff = {
  ...
  update: async (id, clinicId, { name, role_id }) => {
    ...
    await pool.query('UPDATE staff SET name = $1, role_id = $2 WHERE id = $3 AND clinic_id = $4', [
      name,
      role_id,
      id,
      clinicId,
