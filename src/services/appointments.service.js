...
const _checkStaffConflict = async (
  staffId,
  appointmentDate,
  durationMinutes,
  excludeId = null,
  ...
  const endTime = new Date(
    new Date(appointmentDate).getTime() + durationMinutes * 60000,
  );
...
const create = async (
  clinicId,
  { user_id, staff_id, service_id, appointment_date },
) => {
  // Verificar que todos los recursos pertenecen a la clínica
  const [userRes, staffRes, serviceRes, statusRes] = await Promise.all([
    pool.query(
      "SELECT id FROM users WHERE id = $1 AND clinic_id = $2 AND active = true",
      [user_id, clinicId],
    ),
    pool.query(
      "SELECT id FROM staff WHERE id = $1 AND clinic_id = $2 AND active = true",
      [staff_id, clinicId],
    ),
    pool.query(
      "SELECT id, duration_minutes FROM services WHERE id = $1 AND clinic_id = $2 AND active = true",
      [service_id, clinicId],
    ),
    pool.query(
      "SELECT id FROM appointment_status WHERE name = 'pending' AND active = true",
    ),
  ]);

  if (!userRes.rows[0])
    throw new BadRequestError("User not found or inactive in this clinic");
  if (!staffRes.rows[0])
    throw new BadRequestError("Staff not found or inactive in this clinic");
  if (!serviceRes.rows[0])
    throw new BadRequestError("Service not found or inactive in this clinic");

  const durationMinutes = serviceRes.rows[0].duration_minutes;
  const hasConflict = await _checkStaffConflict(
    staff_id,
    appointment_date,
    durationMinutes,
  );
  if (hasConflict)
    throw new ConflictError("Staff already has an appointment at this time");
...
const updateStatus = async (id, clinicId, { status_id }) => {
  ...
  if (appointment.status.is_final) {
    throw new BadRequestError(
      `Cannot change status of a ${appointment.status.name} appointment`,
    );
  }
  ...
  await pool.query("UPDATE appointments SET status_id = $1 WHERE id = $2", [
    status_id,
    id,
  ]);
