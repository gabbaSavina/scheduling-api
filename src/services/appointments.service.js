...
const getById = async (id, clinicId) => {
  const result = await pool.query(`${BASE_QUERY} WHERE a.id = $1 AND a.clinic_id = $2`, [
    id,
    clinicId,
  ]);
...
const create = async (clinicId, { user_id, staff_id, service_id, appointment_date }) => {
  ...
  const [userRes, staffRes, serviceRes, statusRes] = await Promise.all([
    pool.query('SELECT id FROM users WHERE id = $1 AND clinic_id = $2 AND active = true', [
      user_id,
      clinicId,
    ]),
    pool.query('SELECT id FROM staff WHERE id = $1 AND clinic_id = $2 AND active = true', [
      staff_id,
      clinicId,
    ]),
    pool.query(
