...
const positiveIntParam = (field) =>
  param(field).isInt({ gt: 0 }).withMessage(`${field} must be a positive integer`);
...
const appointmentQueryRules = [
  ...
  query('staff_id').optional().isInt({ gt: 0 }).withMessage('staff_id must be a positive integer'),
  query('date')
    .optional()
...
const clinicRules = {
  create: [body('name').trim().notEmpty().withMessage('name is required'), validate],
  getById: [idParam, validate],
  update: [idParam, body('name').trim().notEmpty().withMessage('name is required'), validate],
